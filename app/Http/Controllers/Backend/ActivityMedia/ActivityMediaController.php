...
class ActivityMediaController extends Controller
{
    ...
    /* THIS FUNCTION RETURNS ALL MEDIA IN THE SYSTEM */
    public function AllActiveMedia(ManageActivityMediaRequest $request){
        ...
        if(empty($q)){
            ...
            $medias = Media::select(['medias.id as mId','medias_trans.title as mTitle'])->leftJoin('medias_trans', function($join){
        ...
        foreach ($medias as $key => $value) {
            if(empty($q)){
                ...
                $medias_arr[$value->mId] = $value->mTitle;
            }
        }
