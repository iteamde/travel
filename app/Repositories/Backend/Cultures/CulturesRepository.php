...
namespace App\Repositories\Backend\Cultures;
 ...
 */
class CulturesRepository extends BaseRepository
{
    /**
     * Associated Repository Model.
     */
    const MODEL = Cultures::class;
        ...
        DB::transaction(function () use ($model, $input) {
            ...
            if ($model->save()) {
                foreach ($input as $key => $value) {
                    $trans = new CulturesTranslations;
                    $trans->cultures_id = $model->id;
    ...
    {
        $model = Cultures::findOrFail(['id' => $id]);
        $model = $model[0];

        $prev = CulturesTranslations::where(['cultures_id' => $id])->get();
        if(!empty($prev)){
