...
namespace App\Repositories\Backend\Accommodations;
 ...
 */
class AccommodationsRepository extends BaseRepository
{
    /**
     * Associated Repository Model.
     */
    const MODEL = Accommodations::class;
        ...
        DB::transaction(function () use ($model, $input) {
            ...
            if ($model->save()) {
                foreach ($input as $key => $value) {
                    $trans = new AccommodationsTranslations;
                    $trans->accommodations_id = $model->id;
    ...
    {
        $model = Accommodations::findOrFail(['id' => $id]);
        $model = $model[0];

        $prev = AccommodationsTranslations::where(['accommodations_id' => $id])->get();
        if(!empty($prev)){
