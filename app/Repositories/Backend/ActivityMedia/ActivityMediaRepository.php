...
namespace App\Repositories\Backend\ActivityMedia;
...
/**
 * Class ActivityMediaRepository.
 */
class ActivityMediaRepository extends BaseRepository
{
    /**
    ...
    public function getForDataTable()
    {
        ...
        $dataTableQuery = $this->query()
            ->with('transsingle')
            ->select([
                config('activitymedia.media_table').'.id'
            ]);
        ...
        DB::transaction(function () use ($model, $input) {
            ...
            if ($model->save()) {
                foreach ($input as $key => $value) {
                    $trans = new MediaTranslations;
                    $trans->medias_id = $model->id;
    ...
    {
        $model = Media::findOrFail(['id' => $id]);
        $model = $model[0];

        $model->url = $extra['url'];

        $prev = MediaTranslations::where(['medias_id' => $id])->get();
        if(!empty($prev)){
