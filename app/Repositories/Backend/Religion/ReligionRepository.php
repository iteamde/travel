...
class ReligionRepository extends BaseRepository
{
    ...
    public function getForDataTable()
    {
        ...
        $dataTableQuery = $this->query()
            ...
            ->select([
                ...
                config('religion.religion_table').'.active'
            ]);
        ...
        DB::transaction(function () use ($model, $input) {
            ...
            if ($model->save()) {
                foreach ($input as $key => $value) {
                    $trans = new ReligionTranslations;
                    $trans->religions_id = $model->id;
    ...
    {
        $model = Religion::findOrFail(['id' => $id]);
        $model = $model[0];

        $prev = ReligionTranslations::where(['religions_id' => $id])->get();
        if(!empty($prev)){
