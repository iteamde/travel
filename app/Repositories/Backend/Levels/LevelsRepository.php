...
class LevelsRepository extends BaseRepository
{
        ...
        DB::transaction(function () use ($model, $input) {
            ...
            if ($model->save()) {
                foreach ($input as $key => $value) {
                    $trans = new LevelsTranslations;
                    $trans->levels_id = $model->id;
    ...
    {
        $model = Levels::findOrFail(['id' => $id]);
        $model = $model[0];

        $prev = LevelsTranslations::where(['levels_id' => $id])->get();
        if(!empty($prev)){
