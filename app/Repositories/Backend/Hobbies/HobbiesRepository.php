...
namespace App\Repositories\Backend\Hobbies;
...
class HobbiesRepository extends BaseRepository
{
        ...
        DB::transaction(function () use ($model, $input) {
            ...
            if ($model->save()) {
                foreach ($input as $key => $value) {
                    $trans = new HobbiesTranslations;
                    $trans->hobbies_id = $model->id;
    ...
    {
        $model = Hobbies::findOrFail(['id' => $id]);
        $model = $model[0];

        $prev = HobbiesTranslations::where(['hobbies_id' => $id])->get();
        if(!empty($prev)){
